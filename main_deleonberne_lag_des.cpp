...
const int dim = 5; // number of odes + 1 for function M
// size_t dim = 4;
...
int funcM_2dof(int xRes, int yRes, vector<double> xpx_energybounds, double tau){
    ...
    // Limits of the (x,px) at one fixed coordinate E = 2.100, Fig. 3A1
    // double xMin = -0.400; 
    // double xMax = 0.600;
    // double yMin = -5.800;
    // double yMax = 5.800;
    ...
    // Limits of the (x,px) at one fixed coordinate E = 1.510, Fig. 3A1
    // double xMin = -0.34; 
    // double xMax = 0.48;
    // double yMin = -5.000;
    // double yMax = 5.000;
...
/*
    * Velocity field for dynamics governed by De Leon-Berne system
...
int deleonberne_rhs(double t, const double posIn[], double velOut[], void *params){
    ...
    dV_dx = -( 2*D_X*LAMBDA*exp(-LAMBDA*x)*(exp(-LAMBDA*x) - 1) + (Vdagger/pow(YW,4.0))*ZETA*LAMBDA*pow(y,2.0)*(pow(y,2.0) - 2.0*pow(YW,2.0))*exp(-ZETA*LAMBDA*x) );
